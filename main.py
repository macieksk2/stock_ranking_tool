...
if __name__ == '__main__':
    ...
    if config['use_quant']:
        ...
        for ticker in tickers:
            ...
            income_df[ticker], balance_df[ticker], cash_flow_df[ticker], info[ticker] = data_preprocess(income_df[ticker], balance_df[ticker], cash_flow_df[ticker], info[ticker])
            # Store preprocessed input
    ...
    for ticker in tickers:
        ...
        marketCap, pb_ratio, ps_ratio, pe_ratio, ev_ebitda, beta = retrieve_info_data(info[ticker], config['use_quant'])
        # Calculate scaled volatility of eps --> negative since the higher the worst
        scaled_vol = -np.std(income_df[ticker]['Basic EPS']) / np.mean(abs(income_df[ticker]['Basic EPS']))
        ...
        accrual_rat = -(income_df[ticker]['Net Income'].iloc[0] - cash_flow_df[ticker]['Operating Cash Flow'].iloc[0]) / balance_df[ticker]['Total Assets'].iloc[0]
        # Cash Conversion Ratio
        cash_conv_rat = cash_flow_df[ticker]['Operating Cash Flow'].iloc[0] / income_df[ticker]['Net Income'].iloc[0] 
    ...
    # Normalize the Quant metrics 
    metrics_df = normalize_dataframe(metrics_df, metrics_df.columns, [])
    ...
    final_df['SENTIMENT']   = pd.Series({t: master_scores[t]['Sentiment Score'] for t in tickers})
    
    # Normalize the Qual metrics
    final_df[['MOAT_TEXT', 'MOAT_QUANT', 'MANAGEMENT', 'SENTIMENT']] = normalize_dataframe(
        final_df[['MOAT_TEXT', 'MOAT_QUANT', 'MANAGEMENT', 'SENTIMENT']], 
        ['MOAT_TEXT', 'MOAT_QUANT', 'MANAGEMENT', 'SENTIMENT'], [])
    
    final_df['Qual Score'] = 0  
    for metric, weight in config['qual_scorecard_weights'].items():
        final_df['Qual Score'] += final_df[metric] * weight
